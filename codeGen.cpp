...
bool CodeGenInspector::preorder(const IR::Declaration_Variable* decl) {
    auto type = FPPTypeFactory::instance->create(decl->type);
...
bool CodeGenInspector::comparison(const IR::Operation_Relation* b) {
    ...
    auto et = FPPTypeFactory::instance->create(type);

    bool scalar = (et->is<FPPScalarType>() &&
                   FPPScalarType::generatesScalar(et->to<FPPScalarType>()->widthInBits()));
    if (scalar) {
        builder->append("(");
        visit(b->left);
        builder->spc();
...
bool CodeGenInspector::preorder(const IR::Cast* c) {
    ...
    builder->append("(");
    builder->append("(");
    auto et = FPPTypeFactory::instance->create(c->destType);
    et->emit(builder);
    builder->append(")");
    visit(c->expr);
    builder->append(")");
...
bool CodeGenInspector::preorder(const IR::Type_Typedef* type) {
    auto et = FPPTypeFactory::instance->create(type->type);
...
bool CodeGenInspector::preorder(const IR::AssignmentStatement* a) {
    ...
    auto fppType = FPPTypeFactory::instance->create(ltype);
    bool memcpy = false;
    FPPScalarType* scalar = nullptr;
    unsigned width = 0;
    if (fppType->is<FPPScalarType>()) {
        scalar = fppType->to<FPPScalarType>();
        width = scalar->implementationWidthInBits();
        memcpy = !FPPScalarType::generatesScalar(width);
    }
...
void CodeGenInspector::widthCheck(const IR::Node* node) const {
    ...
    if (tb->size % 8 == 0 && FPPScalarType::generatesScalar(tb->size))
        return;
